# Detector Spec v1 — Pullback Tranche-Entry Backtest
...
2. **Sharpness / efficiency.** Over the impulse leg, `net_displacement / Σ|bar_close_to_close| >= k_efficiency`, with `k_efficiency = 0.55`, **and** leg length `<= L_impulse_max` bars, with `L_impulse_max = 6`. (Efficiency ≈ 1 means a near-straight move; the cap on bar count prevents a slow grind from qualifying.)
3. **Low intra-impulse retracement.** The deepest counter-move *within* the leg satisfies `max_adverse_run <= k_intra × running_extent`, with `k_intra = 0.40`. (No pullback larger than ~40% of progress-so-far inside the impulse itself; larger internal give-back means it is really two legs, not one impulse.)
...
**Counter-consideration (stated, not resolved).** An efficiency-ratio impulse test (criterion 2) can admit a gappy, whippy leg that nets a straight line but is not a clean impulse; conversely it can reject a genuine multi-bar impulse that had one sharp mid-leg breather. The `[OPEN — characterization]` question is whether a single efficiency scalar suffices or whether a max-single-bar-contribution constraint is also needed. v1 uses the single scalar; the alternative is a sensitivity axis, not a v1 branch.
...
| `k_efficiency` (impulse efficiency) | 0.55 | §4 |
| `L_impulse_max` (impulse leg cap, bars) | 6 | §4 |
| `k_intra` (max intra-impulse retrace) | 0.40 | §4 |
